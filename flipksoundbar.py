...
def inser(bar,model,price,rating,review):
	#({'brand':bar,'model':model,str(dt):{'price':int(price),'rating':int(rating),'review':int(review)}})
	#if(doc1 == None):
	#	col.insert_one(doc1)
	#else:
	db1.sndbar.update_one({'brand':bar,'model':model},{"$set": {str(dt):{'price':int(price),'rating':int(rating),'review':int(review)}}},upsert = True)
